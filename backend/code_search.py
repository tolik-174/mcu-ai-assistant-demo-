...
PROJECTS_ROOT = (Path(DATA_DIR) / "projects" / "zips").resolve()
GLOBAL_INDEX_PATH = PROJECTS_ROOT / "global_index.json"
# Розширення кодових файлів
CODE_EXT = {
    ".c", ".h", ".cpp", ".hpp", ".s", ".asm",
...
def simple_similarity(a: str, b: str) -> float:
    """
    Простий fuzzy score 0..100 без зовнішніх бібліотек.
    """
    a = normalize_search_query(a)
    b = normalize_search_query(b)
    ...
    subseq_ratio = i / len(a)
    length_penalty = abs(len(a) - len(b)) / max(len(a), len(b))

    score = (subseq_ratio * 70) - (length_penalty * 20)
    return max(0.0, score)


def score_file_match(query: str, file_info: dict) -> float:
    q = normalize_search_query(query)

    name = str(file_info.get("name") or file_info.get("file") or "").lower()
    path = str(file_info.get("path") or file_info.get("relative_path") or "").lower()
    ...
    fuzzy = max(
        simple_similarity(q, name),
        simple_similarity(q, stem)
    )
    ...
    return 0.0
###############for search testing####################
def local_search_files(filename: str, max_results: int = 80):
    """
    Покращений пошук файлів у локальних індексах.

    Підтримує:
    - exact match: adc_ep.h
    - stem match: adc_ep
    - partial match: adc
    - path match: e2studio adc
    - fuzzy/typo: adcep
    """
    query = normalize_search_query(filename)
    ...
    for proj in projects:
        ...
        for f in index:
            score = score_file_match(query, f)

            if score <= 0:
                continue

            name = f.get("name") or f.get("file") or ""
            rel_path = f.get("path") or f.get("relative_path") or ""
            full_path = f.get("full_path") or f"{proj}/{rel_path}"
    ...
    results.sort(key=lambda r: (-r["score"], len(r["relative_path"])))
